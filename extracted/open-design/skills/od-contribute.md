Source: `.claude/skills/od-contribute/SKILL.md`

This is a Claude Code skill for guiding a (possibly non-coding) user through
contributing to the `nexu-io/open-design` repo — shipping a Skill or Design
System, translating docs, fixing docs/writing a blog post, or filing a bug —
ending in an opened PR (or issue).
...
## Bundled references and templates

Source directory: `.claude/skills/od-contribute/`

### references/

- **design-system-anatomy.md** — Describes the on-disk shape of an OD design-system submission (`design-systems/<brand-slug>/DESIGN.md`, optional `components.html`/`tokens.css`), and the observed `DESIGN.md` section convention (H1 brand name, blockquote pitch, numbered H2 sections), used by `validate-design-system.sh`. Notes the authoritative source is live repo folders, since conventions evolve.
- **newcomer-tone.md** — Voice rules for PR/issue text generated by the skill: always end with a first-contribution greeting plus the OD Discord invite link, never overstate what a PR does, use plain non-jargon language, and limit emoji use. Frames the PR body as the only shot at a good first impression with maintainers.
- **od-repo-map.md** — A cached mirror of `CONTRIBUTING.md`'s "what goes where" table (Skills → `skills/<name>/`, Design Systems → `design-systems/<brand>/DESIGN.md`, Agent adapters → `apps/daemon/src/agents.ts`, docs/i18n → various), plus a table of which localized doc translations exist on disk for README/QUICKSTART/CONTRIBUTING. Notes upstream CONTRIBUTING.md wins if this drifts.
- **skill-anatomy.md** — Describes the on-disk shape of an OD Skill submission (`skills/<name>/SKILL.md` minimum, required YAML frontmatter fields), used by `validate-skill-submission.sh` and for guiding users assembling a Skill submission.

### templates/

- **ISSUE-BODY-bug.md** — Bug-report issue body template with `{{WHAT_HAPPENED}}`, `{{STEPS}}`, `{{EXPECTED}}`, `{{OD_VERSION}}`, `{{PLATFORM}}` placeholders, rendered in Step 3d.
- **PR-BODY-design-system.md** — PR body template for a new Design System submission, with `{{BRAND_NAME}}`, `{{BRAND_SLUG}}`, `{{PITCH}}`, `{{COVERAGE_NOTES}}`, `{{TRY_PROMPT}}`, `{{SCREENSHOT_BLOCK}}` placeholders and try-it instructions (`pnpm tools-dev run web`).
- **PR-BODY-docs.md** — PR body template for docs/typo/blog fixes, with `{{ONE_LINE_SUMMARY}}`, `{{DETAILS}}`, `{{FILES_LIST}}` placeholders.
- **PR-BODY-i18n.md** — PR body template for a translation PR, with `{{DOC_NAME}}`, `{{LANG_DISPLAY_NAME}}`, `{{LANG_CODE}}`, `{{TRANSLATED_PATH}}`, `{{ENGLISH_PATH}}`, `{{STATUS}}`, `{{TRANSLATION_NOTES}}` placeholders, and a checklist of what structure was preserved (code blocks, brand names, cross-links).
- **PR-BODY-skill.md** — PR body template for a new Skill submission, with `{{SKILL_NAME}}`, `{{SKILL_SLUG}}`, `{{PITCH}}`, `{{MOTIVATION}}`, `{{TRY_PROMPT}}`, `{{SCREENSHOT_BLOCK}}` placeholders and try-it instructions (`pnpm tools-dev run web`).
