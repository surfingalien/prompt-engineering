# shareToCommunityPrompt.ts

Source: `apps/web/src/components/share-to-community/shareToCommunityPrompt.ts`

Triggered when a user clicks the post-completion "Share to Open Design"
submission button/action after finishing a project. This is the kickoff
message that gets injected into the agent conversation; it instructs the
agent to package the just-finished project work as an installable Open
Design plugin, following the bundled `od-share-to-community` scenario
(`plugins/_official/scenarios/od-share-to-community/SKILL.md`), and to open a
PR to the community plugin catalog. Per the file's comments, it mirrors the
constraint surface of the `home-hero/plugin-authoring.ts` prompt template
(no chained shell suggestions, no `jq`, point the user at the plugin-folder
card buttons after validate/install) but starts from "introspect the project
the user just finished" rather than a user-typed goal string.

## SHARE_TO_COMMUNITY_PROMPT
...
(Note: in the source, this is built as an array of lines joined with `\n`; the
text above is the fully joined literal result, preserving line breaks and
blank lines exactly as emitted.)
