# media-contract — image/video/audio generation dispatch contract

**Source:** `apps/daemon/src/prompts/media-contract.ts` (canonical/current, 502 lines — much richer). A shorter variant exists at `packages/contracts/src/prompts/media-contract.ts` (74 lines) — included in full below since it's short.

## What triggers/uses this prompt

Exported as `MEDIA_GENERATION_CONTRACT` (daemon: a function-composed template with dynamic model-ID lists and policy scope; contracts: a fixed constant). Rendered via `renderMediaGenerationContract(mediaExecution, byokMediaDefaults)`, injected by the composer (`system.ts`) pinned LAST in the prompt for any **non-web** surface (image / video / audio project), so its hard rules override the softer `<artifact>`-emission wording from `official-system.ts` and the deck/print-stylesheet rules from `deck-framework.ts` and `discovery.ts`. The unifying idea: skill workflow + project metadata tell the agent WHAT to make; a single shell command (`"$OD_NODE_BIN" "$OD_BIN" media generate`) is HOW it actually produces bytes — the agent never fabricates binary content inside `<artifact>` tags.

There's also a `disabled` mode (`mediaExecution.mode === 'disabled'`) that renders a short block instructing the agent NOT to call any OD media dispatcher and to only plan the creative brief in that case.

## Full prompt text (daemon/canonical version, `enabled` mode, no policy-scope/BYOK-defaults overrides)
...
- **image**:   [dynamically interpolated list of registered image model IDs from `IMAGE_MODELS`]
- **video**:   [dynamically interpolated list from `VIDEO_MODELS`]
...
- **audio · music**:  [dynamically interpolated list from `AUDIO_MODELS_BY_KIND.music`]
- **audio · speech**: [dynamically interpolated list from `AUDIO_MODELS_BY_KIND.speech`]
- **audio · sfx**:    [dynamically interpolated list from `AUDIO_MODELS_BY_KIND.sfx`]
...
When `mediaExecution.mode === 'disabled'`, the composer instead renders:
...
[+ optional "Active media policy scope" lines listing allowedSurfaces / allowedModels]
```

---

## Contracts variant (`packages/contracts/src/prompts/media-contract.ts`) — full text

This is a fixed, much shorter constant (no dynamic model-ID interpolation, no policy-scope/BYOK-defaults rendering, no `hyperframes-html` init-scaffold walkthrough, no generate→wait loop pseudocode, no provider-error-signal taxonomy):
...
The contracts version is clearly an earlier/lighter-weight snapshot of the same contract — same environment variables, same CLI shape, same core "never emit `<artifact>` for media" rule — but without the daemon version's operational hardening (retry/poll loop code, per-provider error taxonomy, `hyperframes init` scaffolding walkthrough, dynamic allowed-model-ID catalogue, and the `disabled`-mode policy block).
