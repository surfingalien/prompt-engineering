# discovery — turn-1 clarification, brand-spec extraction, and design philosophy

**Source:** `apps/daemon/src/prompts/discovery.ts` (canonical/current). A near-identical variant exists at `packages/contracts/src/prompts/discovery.ts` — see "Contracts variant" below for the differences.

## What triggers/uses this prompt

Exported as `DISCOVERY_AND_PHILOSOPHY`, rendered via `renderDiscoveryAndPhilosophy(executionProfile)`. This is described in the source comments as "the dominant layer of the composed system prompt" — it is stacked BEFORE the official designer charter (`official-system.ts`) in `system.ts`'s composer, so its hard rules (emit a discovery form on turn 1, branch on brand info on turn 2, plan with TodoWrite on turn 3+) take precedence over the softer wording in the base charter. It is injected for every non-media-surface (HTML/prototype/deck), non-Ask-mode project. It carries three parts:
1. **RULE 1** — turn-1 behavior: emit exactly one `<question-form id="discovery">` (or `id="task-type"` for the default router) and stop.
2. **RULE 2** — turn-2 branching on the submitted `brand` answer (Branch A: brand-spec extraction via Bash/Read/WebFetch; Branch B: skip straight to planning).
3. **RULE 3** — TodoWrite planning discipline, checklist self-check, and 5-dimension critique scoring.
4. A "Design philosophy" section (huashu-distilled) covering persona selection, anti-AI-slop checklist, color/type rules, and cross-platform contracts.

It also exports `renderSharedFramesBlock()`, injected separately by the composer only for multi-device/multi-screen projects, describing the shared `/frames/*.html` device-chrome assets.

A `%%OPEN_DESIGN_HANDOFF_INVARIANT%%` placeholder is substituted with either a filesystem-handoff invariant or a text-artifact-handoff invariant depending on `executionProfile`, mirroring the mechanism in `official-system.ts`.

## Full prompt text (daemon/canonical version, filesystem execution profile)
...
## Filesystem handoff is canonical (dominant-layer invariant)
...
## `renderSharedFramesBlock()` — multi-device shared frames

Injected separately (not part of the main string above) only when the project is multi-target/responsive. Full text:

```
## Multi-device / multi-screen — shared frames
...
## Contracts variant (`packages/contracts/src/prompts/discovery.ts`)

Structurally near-identical (same three RULEs, same forms, same philosophy sections A–H). Differences from the daemon/canonical version:

- No execution-profile placeholder mechanism — the contracts version is fixed to artifact-emission behavior (like its `official-system.ts` counterpart). Where the daemon version says "Turn 3+ … build project files, self-check, and summarize the written files," the contracts version says "…build, self-check, emit `<artifact>` if a new canonical HTML was written this turn (skip on edits-only)."
- The dominant-layer invariant section is titled "## Artifact emission is conditional" instead of "## Filesystem handoff is canonical", with matching artifact-only wording.
- Step 9 of the plan template reads "Emit single `<artifact>` if a new canonical HTML file was written this turn; otherwise summarize the edits" instead of "Summarize the written or changed file(s)…".
- Checklist self-check (step 7) says "Do not emit `<artifact>` with a failing P0" instead of "Do not hand off a filesystem artifact with a failing P0."
- The "5-dimensional critique" section ends with "...re-score. Two passes is normal. Then emit." instead of "...Then finish with a concise file summary," and immediately follows with the direction-library block inline (`${renderDirectionSpecBlock()}` is interpolated directly into `DISCOVERY_AND_PHILOSOPHY` in the contracts version, whereas the daemon version injects the direction spec block separately from the composer in `system.ts`).
- Section ordering differs slightly: the contracts version places "### I. Restraint over ornament" and the "## Default arc (recap)" section AFTER the shared-frames content (there is no separate `renderSharedFramesBlock()` export — the multi-device frames guidance is not present as a discrete function in the contracts file's diffed region), while the daemon version places Restraint (I) before the recap, matching the philosophy section A–I ordering.
- Minor wording diffs: audience placeholder text ("Target user, buyer, viewer, or audience..." vs "e.g. early-stage investors, dev-tools buyers, internal exec review"), body-text minimum in the slide-designer persona bullet (22px vs 24px), and one extra breakpoint (768px) verified in the daemon version's responsive-web contract line that the contracts version omits.
- Contracts version has no `renderDiscoveryAndPhilosophy(executionProfile)` wrapper function — `DISCOVERY_AND_PHILOSOPHY` is exported and consumed directly as a constant.
