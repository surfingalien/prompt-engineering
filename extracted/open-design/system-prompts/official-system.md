# official-system — base "expert designer" identity charter

**Source:** `apps/daemon/src/prompts/official-system.ts` (canonical/current), with a divergent variant at `packages/contracts/src/prompts/official-system.ts` (BYOK/API-only; see "Contracts variant" below).

## What triggers/uses this prompt

This is the root system-prompt charter for every Open Design agent turn — the "expert designer working with the user as manager" identity. It is exported as `OFFICIAL_DESIGNER_PROMPT` and rendered by `renderOfficialDesignerPrompt(executionProfile, options)`, which is called from the composer in `system.ts` (see `system.md`) near the top of every composed prompt, under the heading "# Identity and workflow charter (background)". It defines: the designer persona, the workflow steps, HTML/React output conventions, the React+Babel pinned-CDN setup, deck framework pointer, "Tweaks" panel convention, verification discipline, and IP/copyright guardrails.

Two placeholders (`%%OPEN_DESIGN_EXECUTION_CONTEXT%%` and `%%OPEN_DESIGN_WORKFLOW_HANDOFF%%`) are substituted based on `executionProfile`:
- `filesystem` (default, used by the daemon-backed desktop/web app): the agent writes real project files; no `<artifact>` block is emitted.
- `text_artifact` (used for BYOK/plain-API runs with no filesystem tools): the agent must emit a single `<artifact type="text/html">...</artifact>` block as the deliverable.

There is also a runtime swap: `COPYRIGHT_GUARDRAIL_BULLET` is replaced with `WEB_CLONE_COPYRIGHT_GUARDRAIL_BULLET` when the project's metadata carries `intent: 'web-clone'` — this relaxes the "don't recreate copyrighted designs" rule for explicit Website Clone runs, replacing it with a faithful-reproduction + pre-deploy-replacement-checklist rule.

## Full prompt text (daemon/canonical version, filesystem profile, default guardrail)
...
## Text-artifact profile substitution (BYOK / plain-API runs)

When `executionProfile === 'text_artifact'`, the execution-context and workflow-handoff sections above are replaced with:
...
**Workflow steps 4–5 and handoff block:**
```
4. **Build the artifact.** Compose one complete, standalone HTML document in your response. Inline CSS and JavaScript by default because no filesystem write will happen in this run.
...
## Website Clone guardrail swap

Default bullet (under "What you don't do"):
```
- Don't recreate copyrighted designs (other companies' distinctive UI patterns, branded visual elements). Help the user build something original instead.
```

Swapped in when `metadata.intent === 'web-clone'`:
```
...
## Contracts variant (`packages/contracts/src/prompts/official-system.ts`)

This is an older/fixed variant used by the BYOK/plain-API composer path in `packages/contracts`. Unlike the daemon version, it has **no execution-profile placeholder system** — it is hard-coded to the artifact-emission (`text_artifact`-like) behavior at all times, and always allows React as an explicit output option ("HTML, or React when the user explicitly asks for React output"). Key differences from the daemon/canonical version above:

- Opening line explicitly mentions React as an alternate medium; daemon version defers React mention to the profile-specific workflow.
- Workflow step 1 says "For new or ambiguous work, ask clarifying questions before building" (daemon version defers to the discovery-layer rules).
- Always ends every turn with a mandatory `<artifact identifier="kebab-slug" type="text/html" title="Human title">...</artifact>` block (or `type="text/jsx"` for React), with rules about canonical entry point, no markdown fences, and stopping immediately after `</artifact>`. There is no filesystem-handoff branch.
- Adds an explicit "Do not default to warm beige/cream/peach/pink/orange-brown canvas treatments unless explicitly justified" clause in the Color usage bullet and the AI-slop-tropes bullet — the daemon version's `official-system.ts` does NOT carry this clause directly (it lives instead in the daemon's `discovery.ts` philosophy section and metadata rules).
- Has a dedicated "## Asking good questions" section (start-of-work clarifying-question guidance) that the daemon version omits (the daemon version delegates this entirely to the discovery/question-form layer in `discovery.ts`).
- "## Verification" is a single short paragraph (self-check before emitting the artifact) rather than the daemon's three-part "Verification — converge at the end, in one pass" section with the OD tool-wrapper rendering guidance and "Do not loop" rule.
- Otherwise the identity charter, React+Babel pinned-CDN block, Tweaks-panel convention, and IP guardrail bullets (including the identical `COPYRIGHT_GUARDRAIL_BULLET` / `WEB_CLONE_COPYRIGHT_GUARDRAIL_BULLET` swap mechanism) are byte-identical or near-identical to the daemon version.

The contracts file is explicitly commented as mirroring the daemon file "so API/BYOK and daemon-backed web-clone runs get identical guidance" for the copyright-guardrail swap specifically — the rest of the divergence is legacy drift between the two composer implementations.
