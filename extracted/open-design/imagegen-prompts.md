# Atelier Zero — Image Generation Prompt Pack (open-design-landing skill)

**Source:** `design-templates/open-design-landing/assets/imagegen-prompts.md` and `plugins/_official/examples/open-design-landing/assets/imagegen-prompts.md` — these two files are **byte-identical** (confirmed via `diff`, zero output; the `plugins/_official/examples/` copy is the same landing-page skill repackaged as an installable example plugin). This document reproduces that single file verbatim; see `media-templates/image-prompts.md` and `media-templates/video-prompts.md` for the separate (larger, ~110-item) `prompt-templates/` media-generation catalog — this file is unrelated to that catalog and is specific to generating the marketing images for Open Design's own `open-design-landing` skill/example.
